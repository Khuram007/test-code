...
class UserRepository extends BaseRepository
{
    ...
    public function createOrUpdate($id = null, $request)
    {
        ...
        if ($request['role'] == env('CUSTOMER_ROLE_ID')) {
            ...
            $diff = null;
            if ($request['translator_ex']) $diff = array_intersect($userTranslId, $request['translator_ex']);
            if ($diff || $request['translator_ex']) {
                ...
                if ($blacklistUpdated) UsersBlacklist::deleteFromBlacklist($model->id, $blacklistUpdated);
            } else UsersBlacklist::where('user_id', $model->id)->delete();
            ...
            if ($request['worked_for'] == 'yes') $user_meta->organization_number = $request['organization_number'];
            ...
            if ($request['worked_for'] == 'yes') $data['organization_number'] = $request['organization_number'];
            ...
            if ($request['user_language']) {
                ...
                if ($langidUpdated) $userLang::deleteLang($model->id, $langidUpdated);
            }
        }
        ...
        if ($request['status'] == '1') if ($model->status != '1') $this->enable($model->id);
        else if ($model->status != '0') $this->disable($model->id);
        return $model ?? false;
    }
