...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        return response()->json(['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'cuser' => $cuser, 'usertype' => $usertype], 200);
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs = $jobs_ids;
            $noramlJobs = $jobs_ids;
            return response()->json(['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => $pagenum], 200);
        }
    }
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if (in_array('normal', $data['job_for']) && in_array('certified', $data['job_for'])) $data['certified'] = 'both';
            else if (in_array('normal', $data['job_for']) && in_array('certified_in_law', $data['job_for'])) $data['certified'] = 'n_law';
            else if (in_array('normal', $data['job_for']) && in_array('certified_in_helth', $data['job_for'])) $data['certified'] = 'n_health';
            if ($consumer_type == 'rwsconsumer') $data['job_type'] = 'rws';
            else if ($consumer_type == 'ngo') $data['job_type'] = 'unpaid';
            elseif ($consumer_type == 'paid') $data['job_type'] = 'paid';
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
                            ...
                            if ($job->id == $oneJob->id) { // one potential job is the same with current job
                                ...
                                if ($job_for_translator == 'SpecificJob') {
                                    ...
                                    if (($job_checker != 'userCanNotAcceptJob'))
                                        if ($this->isNeedToDelayPush($oneUser->id)) $delpay_translator_array[] = $oneUser;
                                        else $translator_array[] = $oneUser;
        ...
        return $this->sendPushNotificationToSpecificUsers($delpay_translator_array, $job->id, $data, $msg_text, true); // send new booking push to suitable translators(need to delay)
    }
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        if ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'no') // It's a physical job
            $message = $physicalJobMessageTemplate;
        else if ($job->customer_physical_type == 'no' && $job->customer_phone_type == 'yes') // It's a phone job
            $message = $phoneJobMessageTemplate;
        else if ($job->customer_physical_type == 'yes' && $job->customer_phone_type == 'yes') // It's both, but should be handled as phone job
            $message = $phoneJobMessageTemplate;
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        elseif ($job_type == 'rws') $translator_type = 'rwstranslator';
        elseif ($job_type == 'unpaid') $translator_type = 'volunteer';
    ...
    private function changeCompletedStatus($job, $data)
    {
        ...
        return response()->json($job->save(), 200);
    }
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        return response()->json(['translatorChanged' => $translatorChanged], 200);
    }
    ...
    private function changeDue($old_due, $new_due)
    {
        ...
        return response()->json(['dateChanged' => $dateChanged], 200);
    }
    ...
    public function sendChangedTranslatorNotification($job, $current_translator, $new_translator)
    {
        ...
        $email = $user->email;
        $data['user'] = $user;
        return $this->mailer->send($email, $name, $subject, 'emails.job-changed-translator-new-translator', $data);
    }
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        return $this->mailer->send($translator->email, $translator->name, $subject, 'emails.job-changed-date', $data);
    }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        return $this->sendNotificationTranslator($job, $data, '*');   // send Push all sutiable translators
    }
    ...
    private function sendNotificationChangePending($user, $job, $language, $due, $duration)
    {
        $data = [];
        $data['notification_type'] = 'session_start_remind';
        $msg_text = $job->customer_physical_type == 'yes' ? ["en" => 'Du har nu fått platstolkningen för ' . $language . ' kl ' . $duration . ' den ' . $due . '. Vänligen säkerställ att du är förberedd för den tiden. Tack!']
    ...
    private function getUserTagsStringFromArray($users)
    {
        ...
        $user_tags .= ']';
        return response()->json($user_tags, 200);
    }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        if ($cuser->is('customer')) {
            ...
            $beforeOrAfter = $job->withdraw_at->diffInHours($job->due) >= 24 ? 'before24' : 'after24';
            $job->status = "withdraw$beforeOrAfter";
        ...
        } else if ($job->due->diffInHours(Carbon::now()) > 24) {
...
//                Event::fire(new JobWasCanceled($job));
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if ($limit == 'all') $allJobs = $allJobs->get(); else $allJobs = $allJobs->paginate(15);
        } else {
            $allJobs = Job::query();
            if (isset($requestdata['id']) && $requestdata['id'] != '') {
                $allJobs->where('id', $requestdata['id']);
            ...
            if ($consumer_type == 'RWS') $allJobs->where('job_type', '=', 'rws'); else $allJobs->where('job_type', '=', 'unpaid');
            if (isset($requestdata['feedback']) && $requestdata['feedback'] != 'false') {
                $allJobs->where('ignore_feedback', '0');
                $allJobs->whereHas('feedback', function ($q) {
                    $q->where('rating', '<=', '3');
    ...
    public function alerts()
    {
            ...
            foreach ($jobs as $i => $job) {
                ...
                if (count($sessionTime) >= 3) {
                    ...
                    if ($diff[$i] >= $job->duration)
                        if ($diff[$i] >= $job->duration * 2) $sesJobs [$i] = $job;
                }
            }
        if ($sesJobs) foreach ($sesJobs as $job) $jobId [] = $job->id;
        ...
        return response()->json(['allJobs' => $allJobs, 'languages' => $languages, 'all_customers' => $all_customers, 'all_translators' => $all_translators, 'requestdata' => $requestdata], 200);
    }
    ...
    public function userLoginFailed()
    {
        ...
        return response()->json(['throttles' => $throttles], 200);
    }
    ...
    {
        $languages = $all_customers = $all_translators = [];
        $requestdata = Request::all();
        $cuser = Auth::user();
    ...
    {
        return $this->updateThrottleStatus($id, 'ignore');
    }
    ...
    {
        return $this->updateThrottleStatus($id, 'ignore_expired');
    }
     ...
     * @Author Khuram Qadeer.
     */
    function updateThrottleStatus($id, $key)
    {
